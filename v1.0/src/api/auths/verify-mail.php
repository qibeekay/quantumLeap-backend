...
$userController = new UserController($db, $_ENV['EMAIL_API_URL'], $_ENV['EMAIL_BEARER_TOKEN']);
...
if (Utils::checkAuthorization($config)) {
    ...
    if ($data) {
        ...
        // Verify the email
        $result = $userController->verifyEmail($data);
        // echo json_encode($result);
    } else {
        return self::returnData(false, "Invalid JSON data", null, true);
    }
}